...
private:
	...
	void init(double scale)
	{
		...
		if(p_vc_.norm()==0)
			std::cout << "No measurements received yet to initialize position - using [0 0 0]";
		if((q_cv_.norm()==1) & (q_cv_.w()==1))
			std::cout << "No measurements received yet to initialize attitude - using [1 0 0 0]";
	    ...
	    std::cout << "filter initialized to: \n" <<
